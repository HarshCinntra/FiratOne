...
export class OrderComponent implements OnInit {
  ...
  setPrevious(){
    var filterdata:any[] = this.bridgeService2.getAllFilter('order');
  ...
  setNew(){
    this.bridgeService2.setAllFilter('order',[this.pagination,this.searchValue,this.filter_customer,this.order_by_field,this.order_by_value]);
  }
  getOrders(): void {
    this.isLoading2 = true;
    this.setPrevious();
    this.bridgeService2.getOrderByPagination(this.pagination,this.searchValue,this.filter_customer,this.order_by_field,this.order_by_value).subscribe(
      (data: any) => {
        if (data.status == "200") {
  ...
  gererateinvoice(item: any) {
    ...
    this.bridgeService2.SetOrderType('Order');
    this.route.navigate(['/invoice/add-invoice']);

  }

  createDelivery(item: any) {
    this.bridgeService2.SetOrderId(item);
    this.bridgeService2.setAllFilter('',undefined);
    this.bridgeService2.SetOrderType('Order');
 ...
 private getDismissReason(reason: any): string {
  ...
  cancelOrder(id:any){
      this.bridgeService2.CancelOrder(id).subscribe(
        (res: any) => {
  ...
  GoToPdf2(id: any) {
    const encodedURL = btoa(id);
    const url = "../../assets/html/order2.html?id="+id+this.accesstoken;
    window.open(url, '_blank');
  }
