...
isEdit:boolean=false;
  ...
  this.order.DocDueDate = this.HeadingServices.getPlusDayDate(30);
    ...
    if(this.bridgeService2.getQuotationId() != undefined){
      var QuotID = this.bridgeService2.getQuotationId();
  ...
  getOrder(idd:any): void {
    ...
    this.bridgeService2.getOneOrderdata(idd).subscribe(
      (data: Orders[]) => {
        this.isdataLoading=false;
        var totalamount:any = 0;
        this.quotations = data;
        this.order.id = idd;
        this.order.U_OPPID = Number(this.quotations[0].U_OPPID);
        this.order.U_OPPID = Number(this.quotations[0].U_OPPID);
        ...
        for (let i = 0; i < this.quotations[0].DocumentLines.length; i++) {
          this.QuatItems.push({
            ...
            OpenQuantity: this.quotations[0].DocumentLines[i].OpenQuantity,
            CloseQuantity: Number(this.quotations[0].DocumentLines[i].Quantity) - Number(this.quotations[0].DocumentLines[i].OpenQuantity),
  ...
  getQuotationList(): void {
    ...
    if(this.isEdit == false || this.order.is_draft == 1){
      filterobj = {DocumentStatus:'bost_Open',CancelStatus:'csNo',is_draft:'0'}
    }
    else{
      filterobj = {DocumentStatus:undefined,CancelStatus:undefined,is_draft:undefined}
    }
    this.bridgeService2.getQuotationByPagination({PageNo: 1,maxItem: 'All',},'',filterobj,'id','desc').subscribe(
      (data: any) => {
        this.isLoading2 = false;
  ...
  selectQuotationChangeHandlerItem(event: any) {
    ...
    this.isdataLoading = true;
    this.bridgeService2.getOneQuotationdata(event?.id ?? event).subscribe(
      (data: Quotation[]) => {
        // this.isdataLoading=false;
        this.quotations = data;
        this.order.U_OPPID = Number(this.quotations[0].U_OPPID);
        ...
        this.order.DiscountPercent = Number(this.quotations[0].DiscountPercent);
        this.order.U_OPPRNM = this.quotations[0].U_OPPRNM;
        this.order.U_QUOTID = Number(this.quotations[0].id);
        this.order.U_QUOTNM = this.quotations[0].U_QUOTNM;
        this.order.CardCode = this.quotations[0].CardCode;
        this.order.BPLID = this.quotations[0].BPLID;
  ...
  editItemArray(content: any, item: any,isItemEdit:boolean) {
    ...
    this.ItemOpenQty = Number(item.CloseQuantity) || item.Quantity
    ...
    if (this.CountItem == 0) {
      ...
      for (let i = 0; i < this.QuatItems.length; i++) {
        if (this.ItemCode == this.QuatItems[i]['ItemCode']) {
          check = false;
          this._NotifierService.showError('This item is aready added');
  ...
  minus_qty(item: number) {
    for (let i = 0; i < this.QuatItems.length; i++) {
      if (item == this.QuatItems[i]['ItemCode']) {
      if(this.QuatItems[i]['Quantity'] <= this.QuatItems[i]['CloseQuantity']){

        this._NotifierService.showError('You cannot take Value less than Close Quantity');
      }
        else if (this.QuatItems[i]['Quantity'] > 1) {
          this.QuatItems[i]['Quantity'] -= 1;
          break;
        }
  ...
  addorder(f: NgForm,isdraft:any) {
    if(isdraft == 'draft'){
      this.order.is_draft = 1
    }
    else{
      this.order.is_draft = 0
    }
    ...
    if (this.fl) {
      this.order.Attach = this.fl;
    }
    else {
      this.order.Attach = '';
    }

    if (f.valid || this.order.is_draft == 1) {
      if(this.order.FreightCharge == ''){
      ...
      if(this.total_Amount == undefined || this.total_Amount == ''){
        this.order.DocTotal = 0
      }
      else{
      this.order.DocTotal = this.total_Amount;
      }
      if (this.CountItem == 0 && this.order.is_draft == 0) {
